package com.lyhcoding.myblog.config;
...
public class SecurityConfig {
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        ...
        // 2. frame potion 해제 (시큐리티 h2-console 접속을 허용하기 위해)
        http.headers().frameOptions().disable();

        // 2. Form 로그인 설정
        http.formLogin()
                .loginPage("/loginForm")
                .loginProcessingUrl("/login") // MyUserDetailsService가 호출, Post, x-www-form-urlencoded
